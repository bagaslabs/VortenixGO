...
#if _MSC_VER >= 1910
/* It looks like there were changes as of Visual Studio 2017 and there are no
   32/64 bit versions of _InterlockedExchange[operation], only
   InterlockedExchange[operation] (without leading underscore), so we have to
   distinguish between compiler versions */
...
#define ENET_IPV6 1
static const struct in6_addr enet_v4_anyaddr = {
    {{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff,
      0x00, 0x00, 0x00, 0x00}}};
static const struct in6_addr enet_v4_noaddr = {
    {{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff,
      0xff, 0xff, 0xff, 0xff}}};
static const struct in6_addr enet_v4_localhost = {
    {{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff,
      0x7f, 0x00, 0x00, 0x01}}};
static const struct in6_addr enet_v6_anyaddr = {
    {{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00}}};
static const struct in6_addr enet_v6_noaddr = {
    {{0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
      0xff, 0xff, 0xff, 0xff}}};
static const struct in6_addr enet_v6_localhost = {
    {{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x01}}};
...
typedef struct _ENetCallbacks {
  ...
  ENetPacket *(ENET_CALLBACK *packet_create)(const void *data,
                                             size_t dataLength,
                                             enet_uint32 flags);
...
#define in6_equal(in6_addr_a, in6_addr_b)                                      \
...
typedef enum _ENetPacketFlag {
  ENET_PACKET_FLAG_RELIABLE =
      (1 << 0), /** packet must be received by the target peer and resend
                   attempts should be made until the packet is delivered */
  ENET_PACKET_FLAG_UNSEQUENCED =
      (1 << 1), /** packet will not be sequenced with other packets not
                   supported for reliable packets */
  ENET_PACKET_FLAG_NO_ALLOCATE = (1 << 2), /** packet will not allocate data,
                                              and user must supply it instead */
  ENET_PACKET_FLAG_UNRELIABLE_FRAGMENT =
      (1 << 3), /** packet will be fragmented using unreliable (instead of
                   reliable) sends if it exceeds the MTU */
  ENET_PACKET_FLAG_SENT = (1 << 8), /** whether the packet has been sent from
                                       all queues it has been entered into */
} ENetPacketFlag;
 ...
 *
 *    ENET_PACKET_FLAG_RELIABLE - packet must be received by the target peer and
 ...
 * packets (not supported for reliable packets) ENET_PACKET_FLAG_NO_ALLOCATE -
...
typedef struct _ENetPeer {
  ...
  enet_uint64
      totalPacketsSent; /**< total number of packets sent during a session */
  enet_uint32 packetsLost;
  enet_uint32
      totalPacketsLost; /**< total number of packets lost during a session */
  enet_uint32
...
/** Returns the monotonic time in milliseconds. Its initial value is unspecified
...
/** Attempts to parse the printable form of the IP address in the parameter
   hostName and sets the host field in the address parameter if successful.
        ...
        */
ENET_API int enet_address_set_host_ip_old(ENetAddress *address,
                                          const char *hostName);
...
/** Attempts to resolve the host named by the parameter hostName and sets
                ...
                @returns the address of the given hostName in address on success
        */
ENET_API int enet_address_set_host_old(ENetAddress *address,
                                       const char *hostName);
...
/** Gives the printable form of the IP address specified in the address
   parameter.
                ...
                @returns the null-terminated name of the host in hostName on
   success
                @retval 0 on success
                @retval < 0 on failure
        */
ENET_API int enet_address_get_host_ip_old(const ENetAddress *address,
...
/** Attempts to do a reverse lookup of the host field in the address parameter.
   ...
   success
                ...
                @retval < 0 on failure
        */
ENET_API int enet_address_get_host_old(const ENetAddress *address,
                                       char *hostName, size_t nameLength);
...
ENET_API ENetPacket *enet_packet_create_offset(const void *, size_t, size_t,
                                               enet_uint32);
...
ENET_API int enet_host_send_raw(ENetHost *, const ENetAddress *, enet_uint8 *,
                                size_t);
ENET_API int enet_host_send_raw_ex(ENetHost *host, const ENetAddress *address,
                                   enet_uint8 *data, size_t skipBytes,
                                   size_t bytesToSend);
...
ENET_API void enet_peer_throttle_configure(ENetPeer *, enet_uint32, enet_uint32,
                                           enet_uint32);
ENET_API int enet_host_compress_with_range_coder(ENetHost *);
ENET_API void enet_host_use_crc32(ENetHost *);
ENET_API void enet_packet_destroy(ENetPacket *);
ENET_API void enet_host_use_new_packet(ENetHost *);
ENET_API void enet_host_use_proxy(ENetHost *, const char *, uint16_t,
                                  const char *, const char *);
ENET_API ENetPacket *enet_packet_create(const void *, size_t, enet_uint32);
ENET_API enet_uint32 enet_peer_get_ping(ENetPeer *);
...
#define ENET_IS_SUPPORTED_ATOMIC(size)                                         \
  ENET_AT_CASSERT_PRED(size == 1 || size == 2 || size == 4 || size == 8)
#define ENET_ATOMIC_SIZEOF(variable)                                           \
  (ENET_IS_SUPPORTED_ATOMIC(sizeof(*(variable))), sizeof(*(variable)))
...
__inline int64_t enet_at_atomic_cas(char *ptr, int64_t new_val, int64_t old_val,
                                    size_t size) {
  switch (size) {
  case 1:
    return _InterlockedCompareExchange8((volatile char *)ptr, (char)new_val,
                                        (char)old_val);
  case 2:
  ...
  case 4:
    ...
    return InterlockedCompareExchange((volatile LONG *)ptr, (LONG)new_val,
                                      (LONG)old_val);
#else
    return _InterlockedCompareExchange((volatile LONG *)ptr, (LONG)new_val,
                                       (LONG)old_val);
#endif
  case 8:
#ifdef NOT_UNDERSCORED_INTERLOCKED_EXCHANGE
    return InterlockedCompareExchange64((volatile LONGLONG *)ptr,
                                        (LONGLONG)new_val, (LONGLONG)old_val);
#else
    return _InterlockedCompareExchange64((volatile LONGLONG *)ptr,
                                         (LONGLONG)new_val, (LONGLONG)old_val);
#endif
  default:
    return 0xbad13bad; /* never reached */
...
__inline int64_t enet_at_atomic_inc(char *ptr, int64_t delta,
                                    size_t data_size) {
  switch (data_size) {
  case 1:
...
#define ENET_ATOMIC_READ(variable)                                             \
  enet_at_atomic_read((char *)(variable), ENET_ATOMIC_SIZEOF(variable))
#define ENET_ATOMIC_WRITE(variable, new_val)                                   \
  enet_at_atomic_write((char *)(variable), (int64_t)(new_val),                 \
                       ENET_ATOMIC_SIZEOF(variable))
#define ENET_ATOMIC_CAS(variable, old_value, new_val)                          \
  enet_at_atomic_cas((char *)(variable), (int64_t)(new_val),                   \
                     (int64_t)(old_value), ENET_ATOMIC_SIZEOF(variable))
#define ENET_ATOMIC_INC(variable)                                              \
  enet_at_atomic_inc((char *)(variable), 1, ENET_ATOMIC_SIZEOF(variable))
#define ENET_ATOMIC_DEC(variable)                                              \
  enet_at_atomic_inc((char *)(variable), -1, ENET_ATOMIC_SIZEOF(variable))
...
/* We want to use __atomic built-ins if possible because the __sync primitives
   are deprecated, because the __atomic build-ins allow us to use
   ENET_ATOMIC_WRITE on uninitialized memory without running into undefined
   behavior, and because the
...
#define ENET_ATOMIC_WRITE(ptr, value)                                          \
...
/* clang_analyzer doesn't know that CAS writes to memory so it complains about
          potentially lost data. Replace the code with the equivalent non-sync
   code. */
...
/* Could use __auto_type instead of typeof but that shouldn't work in C++.
                 The ({ }) syntax is a GCC extension called statement
   expression. It lets us return a value out of the macro.

                 TODO We should return bool here instead of the old value to
   avoid the ABA problem. */
#define ENET_ATOMIC_CAS(ptr, old_value, new_value)                             \
  ({                                                                           \
    typeof(*(ptr)) ENET_ATOMIC_CAS_expected_ = (old_value);                    \
    __atomic_compare_exchange_n((ptr), &ENET_ATOMIC_CAS_expected_,             \
                                (new_value), false, __ATOMIC_ACQ_REL,          \
                                __ATOMIC_ACQUIRE);                             \
...
#define ENET_ATOMIC_INC_BY(ptr, delta)                                         \
  __atomic_fetch_add((ptr), (delta), __ATOMIC_ACQ_REL)
#define ENET_ATOMIC_DEC_BY(ptr, delta)                                         \
  __atomic_fetch_sub((ptr), (delta), __ATOMIC_ACQ_REL)
...
#define ENET_ATOMIC_INC_BY(variable, delta)                                    \
  __sync_fetch_and_add((variable), (delta), 1)
#define ENET_ATOMIC_DEC_BY(variable, delta)                                    \
  __sync_fetch_and_sub((variable), (delta), 1)
...
static ENetCallbacks callbacks = {malloc, free, abort, enet_packet_create,
                                  enet_packet_destroy};
...
ENetListIterator enet_list_move(ENetListIterator position, void *dataFirst,
                                void *dataLast) {
...
size_t enet_list_size(ENetList *list) {
  ...
  for (position = enet_list_begin(list); position != enet_list_end(list);
 ...
 * @param data         initial contents of the packet's data; the packet's data
 * will remain uninitialized if data is NULL.
 * @param dataLength   size of the data allocated for this packet
 * @param flags        flags for this packet as described for the ENetPacket
 * structure.
 * @returns the packet on success, NULL on failure
 */
ENetPacket *enet_packet_create(const void *data, size_t dataLength,
                               enet_uint32 flags) {
  ENetPacket *packet;
...
ENetPacket *enet_packet_create_offset(const void *data, size_t dataLength,
                                      size_t dataOffset, enet_uint32 flags) {
  ENetPacket *packet;
  if (flags & ENET_PACKET_FLAG_NO_ALLOCATE) {
    packet = (ENetPacket *)enet_malloc(sizeof(ENetPacket));
    if (packet == NULL) {
    ...
    packet =
        (ENetPacket *)enet_malloc(sizeof(ENetPacket) + dataLength + dataOffset);
    if (packet == NULL) {
      return NULL;
    }
...
static void enet_protocol_notify_disconnect_timeout(ENetHost *host,
                                                    ENetPeer *peer,
                                                    ENetEvent *event) {
  if (peer->state >= ENET_PEER_STATE_CONNECTION_PENDING) {
    host->recalculateBandwidthLimits = 1;
    ...
    if (currentPeer->state == ENET_PEER_STATE_DISCONNECTED) {
      if (peer == NULL) {
        peer = currentPeer;
               ...
               in6_equal(currentPeer->address.host,
...
static int enet_protocol_handle_incoming_commands(ENetHost *host,
  ...
  while (currentData < &host->receivedData[host->receivedDataLength]) {
    ...
    if (peer == NULL &&
        (commandNumber != ENET_PROTOCOL_COMMAND_CONNECT ||
         currentData < &host->receivedData[host->receivedDataLength])) {
      break;
    }
...
static int enet_protocol_receive_incoming_commands(ENetHost *host,
  ...
  for (packets = 0; packets < 256; ++packets) {
    ...
    if (host->usingProxy) {
      memmove(buffer.data, (char *)buffer.data + sizeof(ENetSocks5Header),
  ...
  while (currentCommand != enet_list_end(&peer->outgoingUnreliableCommands)) {
    ...
    if (outgoingCommand->packet != NULL &&
      ...
      if (peer->packetThrottleCounter > peer->packetThrottle) {
        ...
        for (;;) {
          ...
          if (currentCommand ==
              enet_list_end(&peer->outgoingUnreliableCommands)) {
            break;
          }
    ...
    *command = outgoingCommand->command;
    ...
    if (outgoingCommand->packet != NULL) {
      ...
      enet_list_insert(enet_list_end(&peer->sentUnreliableCommands),
                       outgoingCommand);
    } else {
      enet_free(outgoingCommand);
    }
...
static int enet_protocol_check_timeouts(ENetHost *host, ENetPeer *peer,
  ...
  while (currentCommand != enet_list_end(&peer->sentReliableCommands)) {
    ...
    outgoingCommand->roundTripTimeoutLimit =
        peer->timeoutLimit * outgoingCommand->roundTripTimeout;
...
static int enet_protocol_send_outgoing_commands(ENetHost *host,
  ...
  size_t packetSize = host->usingNewPacket ? sizeof(ENetProtocolHeaderUbisoft)
                                           : sizeof(ENetProtocolHeader);
  enet_uint8
      headerData[sizeof(ENetProtocolHeaderUbisoft) + sizeof(enet_uint32)] = {0};
  ENetProtocolHeader *header = (ENetProtocolHeader *)headerData;
  ENetProtocolHeaderUbisoft *newHeader =
      (ENetProtocolHeaderUbisoft *)headerData;
    ...
    for (host->continueSending = 0, currentPeer = host->peers;
      ...
      if (currentPeer->packetLossEpoch == 0)
        ...
        if (packetLoss >= currentPeer->packetLoss) {
          ...
          currentPeer->packetLossVariance +=
              (packetLoss - currentPeer->packetLoss) / 4;
        } else {
          currentPeer->packetLoss -= (currentPeer->packetLoss - packetLoss) / 8;
          currentPeer->packetLossVariance +=
              (currentPeer->packetLoss - packetLoss) / 4;
        }
      ...
      if (host->headerFlags & ENET_PROTOCOL_HEADER_FLAG_SENT_TIME) {
        ...
        if (host->usingNewPacket) {
          host->buffers->dataLength =
              (size_t)&((ENetProtocolHeaderUbisoft *)0)->sentTime;
        } else {
          host->buffers->dataLength =
              (size_t)&((ENetProtocolHeader *)0)->sentTime;
        }
      ...
      if (host->usingProxy) {
        memmove(&host->buffers[1], &host->buffers[0],
                sizeof(host->buffers) - sizeof(host->buffers[0]));
        ++host->bufferCount;
...
int enet_host_service(ENetHost *host, ENetEvent *event, enet_uint32 timeout) {
  ...
  if (host->usingProxy && host->proxyState != ENET_SOCKS5_STATE_CONNECTED) {
    ...
    case ENET_SOCKS5_STATE_SEND_AUTH_REQUEST: {
      ...
      authReq.authMethods[0] = (host->proxyInfo.auth.username == NULL ||
                                host->proxyInfo.auth.password == NULL)
                                   ? ENET_SOCKS5_AUTH_UNAUTHORIZED
                                   : ENET_SOCKS5_AUTH_USERNAME_PASSWORD;

      buffer.data = &authReq;
      buffer.dataLength = sizeof(authReq) - sizeof(authReq.authMethods) +
                          authReq.authMethodCount;

      if (enet_socket_send(host->proxySocket, &host->proxyAddress, &buffer, 1) >
          0)
    ...
    case ENET_SOCKS5_STATE_RECEIVE_AUTH_RESPONSE: {
      ...
      if (enet_socket_receive(host->proxySocket, &host->proxyAddress, &buffer,
                              1) > 0) {
        if (authRes.version != ENET_SOCKS5_VERSION_5 ||
            (authRes.authMethod != ENET_SOCKS5_AUTH_UNAUTHORIZED &&
    ...
    case ENET_SOCKS5_STATE_SEND_AUTH_REQUEST_USERNAME: {
      ...
      memcpy((char *)&authReqUsername.username + authReqUsername.usernameLength,
             &pass_len, 1);
      strcpy((char *)&authReqUsername.username +
                 authReqUsername.usernameLength + 1,
             host->proxyInfo.auth.password);

      buffer.data = &authReqUsername;
      buffer.dataLength = sizeof(authReqUsername) - 255 * 2 +
                          authReqUsername.usernameLength + pass_len;
    ...
    case ENET_SOCKS5_STATE_RECEIVE_AUTH_RESPONSE_USERNAME: {
      ...
      if (enet_socket_receive(host->proxySocket, &host->proxyAddress, &buffer,
                              1) > 0) {
        if (authResUsername.version != 1 ||
            authResUsername.authMethod != ENET_SOCKS5_REPLY_SUCCEEDED) {
          host->proxyState = ENET_SOCKS5_STATE_CONNECTION_FAILED;
        } else {
          host->proxyState = ENET_SOCKS5_STATE_SEND_REQUEST;
        }
    ...
    case ENET_SOCKS5_STATE_RECEIVE_RESPONSE: {
      ...
      if (enet_socket_receive(host->proxySocket, &host->proxyAddress, &buffer,
        ...
        if (res.version != ENET_SOCKS5_VERSION_5 ||
          ...
          enet_inaddr_map4to6(*(struct in_addr *)&res.ipv4.addr,
                              &host->address.host);
 ...
 * ENet to the peer. The lowest mean round trip time from the sending of a
 * reliable packet to the receipt of its acknowledgement is measured over an
 * amount of time specified by the interval parameter in milliseconds.  If a
 * measured round trip time happens to be significantly less than the mean round
 * trip time measured over the interval, then the throttle probability is
 * increased to allow more traffic by an amount specified in the acceleration
 * parameter, which is a ratio to the ENET_PEER_PACKET_THROTTLE_SCALE constant.
 * If a measured round trip time happens to be significantly greater than the
 ...
 * ENET_PEER_PACKET_THROTTLE_SCALE constant.  When the throttle has a value of
 * ENET_PEER_PACKET_THROTTLE_SCALE, no unreliable packets are dropped by ENet,
 * and so 100% of all unreliable packets will be sent.  When the throttle has a
 *  value of 0, all unreliable packets are dropped by ENet, and so 0% of all
 * unreliable packets will be sent.  Intermediate values for the throttle
 * represent intermediate probabilities between 0% and 100% of unreliable
 * packets being sent.  The bandwidth limits of the local and foreign hosts are
 * taken into account to determine a sensible limit for the throttle probability
 * above which it should not raise even in the best of conditions.
 *
 *  @param peer peer to configure
 *  @param interval interval, in milliseconds, over which to measure lowest mean
 * RTT; the default value is ENET_PEER_PACKET_THROTTLE_INTERVAL.
 *  @param acceleration rate at which to increase the throttle probability as
 * mean RTT declines
 *  @param deceleration rate at which to decrease the throttle probability as
 * mean RTT increases
 */
void enet_peer_throttle_configure(ENetPeer *peer, enet_uint32 interval,
  ...
  command.throttleConfigure.packetThrottleInterval =
...
int enet_peer_throttle(ENetPeer *peer, enet_uint32 rtt) {
  if (peer->lastRoundTripTime <= peer->lastRoundTripTimeVariance) {
    ...
    return 1;
             ...
             peer->lastRoundTripTime + 2 * peer->lastRoundTripTimeVariance) {
 ...
 */
enet_uint32 enet_host_get_received_data(ENetHost *host,
                                        /*out*/ enet_uint8 **data) {
  *data = host->receivedData;
  return host->receivedDataLength;
}

enet_uint32 enet_host_get_mtu(ENetHost *host) { return host->mtu; }

enet_uint32 enet_peer_get_id(ENetPeer *peer) { return peer->connectID; }
...
enet_uint16 enet_peer_get_port(ENetPeer *peer) { return peer->address.port; }

ENetPeerState enet_peer_get_state(ENetPeer *peer) { return peer->state; }

enet_uint32 enet_peer_get_rtt(ENetPeer *peer) { return peer->roundTripTime; }
...
void *enet_peer_get_data(ENetPeer *peer) { return (void *)peer->data; }
...
int enet_peer_send(ENetPeer *peer, enet_uint8 channelID, ENetPacket *packet) {
  ...
  if (peer->host->usingNewPacket) {
    fragmentLength = peer->mtu - sizeof(ENetProtocolHeaderUbisoft) -
                     sizeof(ENetProtocolSendFragment);
  } else {
  ...
  if (packet->dataLength > fragmentLength) {
    enet_uint32 fragmentCount =
                    (packet->dataLength + fragmentLength - 1) / fragmentLength,
    ...
    for (fragmentNumber = 0, fragmentOffset = 0;
      ...
      fragment->command.sendFragment.dataLength =
      ...
      fragment->command.sendFragment.totalLength =
          ENET_HOST_TO_NET_32(packet->dataLength);
    ...
    while (!enet_list_empty(&fragments)) {
      fragment =
          (ENetOutgoingCommand *)enet_list_remove(enet_list_begin(&fragments));
  ...
  if ((packet->flags &
    ...
    command.sendUnsequenced.dataLength =
        ENET_HOST_TO_NET_16(packet->dataLength);
  ...
  if (enet_peer_queue_outgoing_command(peer, &command, packet, 0,
 ...
 *  @param channelID holds the channel ID of the channel the packet was received
 * on success
 *  @returns a pointer to the packet, or NULL if there are no available incoming
 * queued packets
 */
...
void enet_peer_reset_outgoing_commands(ENetList *queue) {
  ...
  while (!enet_list_empty(queue)) {
    outgoingCommand =
        (ENetOutgoingCommand *)enet_list_remove(enet_list_begin(queue));
...
static void enet_peer_remove_incoming_commands(ENetList *queue,
...
static void enet_peer_reset_incoming_commands(ENetList *queue) {
  enet_peer_remove_incoming_commands(queue, enet_list_begin(queue),
                                     enet_list_end(queue));
}
...
void enet_peer_reset_queues(ENetPeer *peer) {
  ...
  if (peer->channels != NULL && peer->channelCount > 0) {
    for (channel = peer->channels;
         channel < &peer->channels[peer->channelCount]; ++channel) {
...
void enet_peer_reset(ENetPeer *peer) {
  ...
  // We don't want to reset connectID here, otherwise, we can't get it in the
  // Disconnect event peer->connectID                     = 0;
  peer->outgoingPeerID = ENET_PROTOCOL_MAXIMUM_PEER_ID;
 ...
 *  @remarks ping requests factor into the mean round trip time as designated by
 * the roundTripTime field in the ENetPeer structure.  ENet automatically pings
 * all connected peers at regular intervals, however, this function may be
 ...
 *  @param pingInterval the interval at which to send pings; defaults to
 * ENET_PEER_PING_INTERVAL if 0
 */
 ...
 * backoff mechanism, where if a reliable packet is not acknowledge within some
 * multiple of the average RTT plus a variance tolerance, the timeout will be
 * doubled until it reaches a set limit. If the timeout is thus at this limit
 * and reliable packets have been sent but not acknowledged within a certain
 * minimum time period, the peer will be disconnected. Alternatively, if
 * reliable packets have been sent but not acknowledged for a certain maximum
 * time period, the peer will be disconnected regardless of the current timeout
 * limit value.
 *
 *  @param peer the peer to adjust
 *  @param timeoutLimit the timeout limit; defaults to ENET_PEER_TIMEOUT_LIMIT
 * if 0
 *  @param timeoutMinimum the timeout minimum; defaults to
 * ENET_PEER_TIMEOUT_MINIMUM if 0
 *  @param timeoutMaximum the timeout maximum; defaults to
 * ENET_PEER_TIMEOUT_MAXIMUM if 0
 */

void enet_peer_timeout(ENetPeer *peer, enet_uint32 timeoutLimit,
                       enet_uint32 timeoutMinimum, enet_uint32 timeoutMaximum) {
  peer->timeoutLimit = timeoutLimit ? timeoutLimit : ENET_PEER_TIMEOUT_LIMIT;
  peer->timeoutMinimum =
      timeoutMinimum ? timeoutMinimum : ENET_PEER_TIMEOUT_MINIMUM;
  peer->timeoutMaximum =
      timeoutMaximum ? timeoutMaximum : ENET_PEER_TIMEOUT_MAXIMUM;
}

/** Force an immediate disconnection from a peer.
 *  @param peer peer to disconnect
 *  @param data data describing the disconnection
 *  @remarks No ENET_EVENT_DISCONNECT event will be generated. The foreign peer
 * is not guaranteed to receive the disconnect notification, and is reset
 ...
 *  @param data data describing the disconnection
 *  @remarks An ENET_EVENT_DISCONNECT event will be generated by
 * enet_host_service() once the disconnection is complete.
 */
...
ENetAcknowledgement *
enet_peer_queue_acknowledgement(ENetPeer *peer, const ENetProtocol *command,
                                enet_uint16 sentTime) {
  ENetAcknowledgement *acknowledgement;
  ...
  if (command->header.channelID < peer->channelCount) {
    ...
    enet_uint16 reliableWindow =
        command->header.reliableSequenceNumber / ENET_PEER_RELIABLE_WINDOW_SIZE;
    enet_uint16 currentWindow = channel->incomingReliableSequenceNumber /
                                ENET_PEER_RELIABLE_WINDOW_SIZE;
...
void enet_peer_setup_outgoing_command(ENetPeer *peer,
  ...
  switch (outgoingCommand->command.header.command &
          ENET_PROTOCOL_COMMAND_MASK) {
  case ENET_PROTOCOL_COMMAND_SEND_UNRELIABLE:
  ...
  if (outgoingCommand->command.header.command &
      ENET_PROTOCOL_COMMAND_FLAG_ACKNOWLEDGE) {
    enet_list_insert(enet_list_end(&peer->outgoingReliableCommands),
                     outgoingCommand);
  } else {
    enet_list_insert(enet_list_end(&peer->outgoingUnreliableCommands),
                     outgoingCommand);
  }
}

ENetOutgoingCommand *
enet_peer_queue_outgoing_command(ENetPeer *peer, const ENetProtocol *command,
...
void enet_peer_dispatch_incoming_reliable_commands(ENetPeer *peer,
  ...
  for (currentCommand = enet_list_begin(&channel->incomingReliableCommands);
    ...
    if (incomingCommand->fragmentCount > 0) {
      channel->incomingReliableSequenceNumber +=
          incomingCommand->fragmentCount - 1;
    }
  }
...
ENetIncomingCommand *
enet_peer_queue_incoming_command(ENetPeer *peer, const ENetProtocol *command,
                                 const void *data, size_t dataLength,
                                 enet_uint32 flags, enet_uint32 fragmentCount) {
  ...
  incomingCommand =
      (ENetIncomingCommand *)enet_malloc(sizeof(ENetIncomingCommand));
  ...
  if (fragmentCount > 0) {
    ...
    memset(incomingCommand->fragments, 0,
           (fragmentCount + 31) / 32 * sizeof(enet_uint32));
  }
 ...
 * time.
 */
ENetHost *enet_host_create(const ENetAddress *address, size_t peerCount,
...
ENetPeer *enet_host_connect(ENetHost *host, const ENetAddress *address,
  ...
  if (host->usingProxy) {
    ...
    enet_inaddr_map6to4(&address->host,
                        (struct in_addr *)&host->proxyHeader.ipv4.addr);
...
/** Sends raw data to specified address. Useful when you want to send
 * unconnected data using host's socket.
 ...
 *  @sa enet_socket_send
 */
int enet_host_send_raw(ENetHost *host, const ENetAddress *address,
                       enet_uint8 *data, size_t dataLength) {
  ENetBuffer buffer;
  buffer.data = data;
...
/** Sends raw data to specified address with extended arguments. Allows to send
 * only part of data, handy for other programming languages. I.e. if you have
 * data =- { 0, 1, 2, 3 } and call function as enet_host_send_raw_ex(data, 1, 2)
 * then it will skip 1 byte and send 2 bytes { 1, 2 }.
 *  @param host host sending data
 *  @param address destination address
 *  @param data data pointer
 ...
 *  @sa enet_socket_send
 */
int enet_host_send_raw_ex(ENetHost *host, const ENetAddress *address,
                          enet_uint8 *data, size_t skipBytes,
                          size_t bytesToSend) {
  ENetBuffer buffer;
 ...
 */
void enet_host_set_intercept(ENetHost *host,
                             const ENetInterceptCallback callback) {
...
#define clock_gettime enet_clock_gettime
...
static int enet_in6addr_lookup_host(const char *name, bool nodns,
                                    ENetAddress *out) {
  ...
  for (result = resultList; result != NULL; result = result->ai_next) {
    if (result->ai_addr != NULL) {
      if (result->ai_family == AF_INET ||
        ...
        enet_inaddr_map4to6(((struct sockaddr_in *)result->ai_addr)->sin_addr,
                            &out->host);
        out->sin6_scope_id = 0;
        ...
        memcpy(&out->host, &((struct sockaddr_in6 *)result->ai_addr)->sin6_addr,
               sizeof(struct in6_addr));
        out->sin6_scope_id =
            (enet_uint16)((struct sockaddr_in6 *)result->ai_addr)
                ->sin6_scope_id;
...
int enet_address_get_host(const ENetAddress *address, char *name,
  ...
  err = getnameinfo((struct sockaddr *)&sin, sizeof(sin), name, nameLength,
                    NULL, 0, NI_NAMEREQD);
  if (!err) {
...
enet_uint64 enet_host_random_seed(void) { return (enet_uint64)time(NULL); }
...
int enet_address_get_host_ip_old(const ENetAddress *address, char *name,
                                 size_t nameLength) {
  if (inet_ntop(AF_INET6, &address->host, name, nameLength) == NULL) {
    return -1;
  }
...
int enet_address_get_host_old(const ENetAddress *address, char *name,
                              size_t nameLength) {
  struct sockaddr_in6 sin;
  int err;
  memset(&sin, 0, sizeof(struct sockaddr_in6));
...
ENetSocket enet_socket_create(ENetSocketType type) {
  return socket(PF_INET6,
                type == ENET_SOCKET_TYPE_DATAGRAM ? SOCK_DGRAM : SOCK_STREAM,
                0);
}
...
int enet_socket_set_option(ENetSocket socket, ENetSocketOption option,
  ...
  case ENET_SOCKOPT_RCVBUF:
    result =
        setsockopt(socket, SOL_SOCKET, SO_RCVBUF, (char *)&value, sizeof(int));
    break;

  case ENET_SOCKOPT_SNDBUF:
    result =
        setsockopt(socket, SOL_SOCKET, SO_SNDBUF, (char *)&value, sizeof(int));
    break;
  ...
  case ENET_SOCKOPT_IPV6_V6ONLY:
    result = setsockopt(socket, IPPROTO_IPV6, IPV6_V6ONLY, (char *)&value,
                        sizeof(int));
    break;
...
int enet_socket_connect(ENetSocket socket, const ENetAddress *address) {
  ...
  result =
      connect(socket, (struct sockaddr *)&sin, sizeof(struct sockaddr_in6));
...
enet_uint64 enet_host_random_seed(void) { return (enet_uint64)timeGetTime(); }
...
int enet_address_set_host_old(ENetAddress *address, const char *name) {
  ...
  ((enet_uint32 *)&address->host.s6_addr)[3] =
      *(enet_uint32 *)hostEntry->h_addr_list[0];
...
int enet_socket_listen(ENetSocket socket, int backlog) {
  return listen(socket, backlog < 0 ? SOMAXCONN : backlog) == SOCKET_ERROR ? -1
                                                                           : 0;
}
...
int enet_socket_send(ENetSocket socket, const ENetAddress *address,
  ...
  if (WSASendTo(socket, (LPWSABUF)buffers, (DWORD)bufferCount, &sentLength, 0,
                ...
                address != NULL ? sizeof(struct sockaddr_in6) : 0, NULL,
                NULL) == SOCKET_ERROR) {
...
/* adaptation constants tuned aggressively for small packet sizes rather than
...
typedef struct _ENetRangeCoder {
  /* only allocate enough symbols for reasonable MTUs, would need to be larger
   * for large file compression */
  ENetSymbol symbols[4096];
} ENetRangeCoder;

static void *enet_range_coder_create(void) {
  ENetRangeCoder *rangeCoder =
      (ENetRangeCoder *)enet_malloc(sizeof(ENetRangeCoder));
  if (rangeCoder == NULL)
    return NULL;
...
static void enet_range_coder_destroy(void *context) {
...
#define ENET_CONTEXT_RESCALE(context, minimum)                                 \
  {                                                                            \
    (context)->total =                                                         \
        (context)->symbols                                                     \
            ? enet_symbol_rescale((context) + (context)->symbols)              \
            : 0;                                                               \
...
#define ENET_RANGE_CODER_FREE_SYMBOLS                                          \
  {                                                                            \
    if (nextSymbol >= sizeof(rangeCoder->symbols) / sizeof(ENetSymbol) -       \
                          ENET_SUBCONTEXT_ORDER) {                             \
      nextSymbol = 0;                                                          \
      ENET_CONTEXT_CREATE(root, ENET_CONTEXT_ESCAPE_MINIMUM,                   \
...
#define ENET_CONTEXT_ENCODE_EXCLUDE(context, value_, under, total, minimum)    \
  ENET_CONTEXT_WALK(context, {                                                 \
    if (node->value != value_) {                                               \
      enet_uint16 parentCount =                                                \
          rangeCoder->symbols[node->parent].count + minimum;                   \
      if (node->value < value_)                                                \
        under -= parentCount;                                                  \
      total -= parentCount;                                                    \
    }                                                                          \
...
static size_t enet_range_coder_compress(void *context,
  ...
  for (;;) {
...
#ifdef ENET_CONTEXT_EXCLUSION
      if (childContext->total >
          ENET_SUBCONTEXT_SYMBOL_DELTA + ENET_SUBCONTEXT_ESCAPE_DELTA)
    ...
    ENET_CONTEXT_ENCODE(root, symbol, value, under, count,
    ...
    *parent = symbol - rangeCoder->symbols;
    ...
    if (childContext->total >
        ENET_SUBCONTEXT_SYMBOL_DELTA + ENET_SUBCONTEXT_ESCAPE_DELTA)
      ENET_CONTEXT_ENCODE_EXCLUDE(childContext, value, under, total,
                                  ENET_CONTEXT_SYMBOL_MINIMUM);
#endif
    ENET_RANGE_CODER_ENCODE(root->escapes + under, count, total);
    root->total += ENET_CONTEXT_SYMBOL_DELTA;
    if (count > 0xFF - 2 * ENET_CONTEXT_SYMBOL_DELTA +
                    ENET_CONTEXT_SYMBOL_MINIMUM ||
...
#define ENET_RANGE_CODER_READ(total)                                           \
  ((decodeCode - decodeLow) / (decodeRange /= (total)))
...
#define ENET_CONTEXT_DECODE(context, symbol_, code, value_, under_, count_,    \
                            update, minimum, createRoot, visitNode,            \
  ...
  {                                                                            \
    ...
    if (!(context)->symbols) {                                                 \
      ...
      for (;;) {                                                               \
        enet_uint16 after =                                                    \
                        under_ + node->under + (node->value + 1) * minimum,    \
...
#define ENET_CONTEXT_TRY_DECODE(context, symbol_, code, value_, under_,        \
                                count_, update, minimum, exclude)              \
  ENET_CONTEXT_DECODE(context, symbol_, code, value_, under_, count_, update,  \
                      minimum, return 0, exclude(node->value, after, before),  \
                      return 0, return 0)

#define ENET_CONTEXT_ROOT_DECODE(context, symbol_, code, value_, under_,       \
                                 count_, update, minimum, exclude)             \
  ENET_CONTEXT_DECODE(                                                         \
...
static size_t enet_range_coder_decompress(void *context,
  ...
  for (;;) {
...
#ifdef ENET_CONTEXT_EXCLUSION
    if (childContext->total > 0)
      ENET_CONTEXT_DECODE_EXCLUDE(childContext, total,
                                  ENET_CONTEXT_SYMBOL_MINIMUM);
#endif
    code = ENET_RANGE_CODER_READ(total);
    ...
    if (childContext->total > 0) {
      ENET_CONTEXT_ROOT_DECODE(
          root, symbol, code, value, under, count, ENET_CONTEXT_SYMBOL_DELTA,
  ...
  patchContexts:
    for (patch = &rangeCoder->symbols[predicted]; patch != subcontext;
         patch = &rangeCoder->symbols[patch->parent]) {
      ENET_CONTEXT_ENCODE(patch, symbol, value, under, count,
                          ENET_SUBCONTEXT_SYMBOL_DELTA, 0);
      *parent = symbol - rangeCoder->symbols;
      parent = &symbol->parent;
      if (count <= 0) {
...
void enet_host_use_crc32(ENetHost *host) { host->checksum = enet_crc32; }

void enet_host_use_new_packet(ENetHost *host) { host->usingNewPacket = 1; }

void enet_host_use_proxy(ENetHost *host, const char *ip, uint16_t port,
                         const char *username, const char *password) {
  host->usingProxy = 1;
  char *paddress = (char *)malloc(strlen(ip) + 1);
  char *pusername = (char *)malloc(strlen(username) + 1);
  char *ppassword = (char *)malloc(strlen(password) + 1);

  strcpy(paddress, ip);
  strcpy(pusername, username);
  strcpy(ppassword, password);

  host->proxyInfo.ip = paddress;
  host->proxyInfo.port = port;
  host->proxyInfo.auth.username = pusername;
  host->proxyInfo.auth.password = ppassword;
}

enet_uint32 enet_peer_get_ping(ENetPeer *peer) { return peer->roundTripTime; }
